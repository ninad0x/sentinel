...
export default function IncidentTimeline({ data }: MonitorProps) {
  return (
    <div>
      <SectionHeader text="Incident timeline" />

      {!data.incidents.length ? (
        <p className="text-xs font-mono text-gray-400 px-8 py-6">
              ...
              {data.incidents.map((incident) => {
                ...
                return (
                  <div key={incident.id} className="relative flex gap-6">
                    {/* Dot */}
                    <div
                      className={`relative z-10 mt-1 w-3.5 h-3.5 rounded-full border-2 shrink-0 ${
                        isResolved
                    ...
                    <div className="flex justify-between items-center w-full pb-2">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`text-xs font-semibold uppercase tracking-wide ${isResolved ? "text-green-600" : "text-red-500"}`}
                          >
                            {incident.status}
                          </span>
                      ...
                      <div className="flex flex-col items-end">
                        {duration && (
                          <>
                            <p className="font-mono text-xs text-gray-400">duration</p>
